...
DEFINE_SCRIPTFUNC(CheckComplete, "Looks through Progress.csp for a map and returns whether it has been completed or not.")
DEFINE_SCRIPTFUNC(SetMapStatus, "Changes map to complete in Progress.csp.")
END_SCRIPTDESC()
...
void SMSM::SaveCoopSPProgress() {
    std::fstream prog;

    prog.open("C:\\Program Files (x86)\\Steam\\steamapps\\sourcemods\\Portal 2 Coop SP\\Progress.csp", std::ofstream::out | std::ofstream::trunc);
    prog.close();//Erases entire file

    prog.open("C:\\Program Files (x86)\\Steam\\steamapps\\sourcemods\\Portal 2 Coop SP\\Progress.csp");
    for (auto e : progress) {prog << e.first << " " << e.second << "\n";}
    prog.close();
}

void SMSM::SetMapStatus(int mNum, bool status) {
    progress[mNum].second = status;
}

bool SMSM::CheckComplete(int mNum) {
    return progress[mNum].second;
}
...
void SMSM::LoadCoopSPProgress() {
    ...
    while (getline(prog, temp, '\n')) { 
        ...
        progress.push_back(std::pair<std::string, bool>{ temp.substr(0, temp.size() - 2), result});
    }
}

void SMSM::StartMainThread() {
    //this->ForceAct5MenuBackground();
    this->LoadCoopSPProgress();
}
